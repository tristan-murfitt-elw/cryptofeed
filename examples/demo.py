...
from cryptofeed.callback import BookCallback, FundingCallback, TickerCallback, TradeCallback, UnderlyingIndexCallback, OpenInterestCallback
from cryptofeed.defines import CANDLES, BID, ASK, BLOCKCHAIN, COINBASE, FUNDING, GEMINI, L2_BOOK, L3_BOOK, OPEN_INTEREST, TICKER, TRADES, UNDERLYING_INDEX, BOOK_DELTA
from cryptofeed.exchanges import (FTX, Binance, BinanceUS, BinanceFutures, Bitfinex, Bitflyer, Bitmax, Bitmex, Bitstamp, Bittrex, Coinbase, Gateio,
...
async def underlying_index(**kwargs):
    print(f"UnderlyingIndex: {kwargs}")
...
def main():
    ...
    f.add_feed(Bybit(symbols=['BTC-USDT', 'BTC-USD'], channels=[UNDERLYING_INDEX], callbacks={OPEN_INTEREST: OpenInterestCallback(oi), UNDERLYING_INDEX: UnderlyingIndexCallback(underlying_index)}))
