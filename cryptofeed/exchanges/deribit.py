...
class Deribit(Feed, DeribitRestMixin):
    ...
    async def _ticker(self, msg: dict, timestamp: float):
        '''
        {
            ...
            "jsonrpc" : "2.0"}
        '''
        message = msg['params']['data']
        pair = self.exchange_symbol_to_std_symbol(message['instrument_name'])
        ts = self.timestamp_normalize(message['timestamp'])
        extra_fields = {
            'bbo': self.get_book_bbo(pair),
            'high': Decimal(message.get('max_price', 0)),
            'low': Decimal(message.get('min_price', 0)),
            'last': Decimal(message.get('last_price') or 0),   # nullable
            'mark_price': Decimal(message.get('mark_price', 0)),
            'underlying_price': Decimal(message.get('underlying_price', 0)),
            'volume': Decimal(message['stats'].get('volume') or 0),    # nullable
            'best_bid_size': Decimal(message.get('best_bid_amount', 0)),
            'best_ask_size': Decimal(message.get('best_ask_amount', 0)),
            'bid_iv': Decimal(message.get('bid_iv', 0)),
            'ask_iv': Decimal(message.get('ask_iv', 0)),
            'mark_iv': Decimal(message.get('mark_iv', 0)),
            **message.get('greeks', {}),  # vega, theta, rho, gamma, delta
        }
        t = Ticker(
            self.id,
            pair,
        ...
        await self.callback(TICKER, t, timestamp, **extra_fields)
    ...
    async def _book_snapshot(self, msg: dict, timestamp: float):
        ...
        bids = {Decimal(price): Decimal(amount) for _, price, amount in msg["params"]["data"]["bids"]}
        asks = {Decimal(price): Decimal(amount) for _, price, amount in msg["params"]["data"]["asks"]}
