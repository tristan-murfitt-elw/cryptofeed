...
class Kraken(Feed, KrakenRestMixin):
    ...
    def __init__(self, candle_interval='1m', max_depth=0, **kwargs):
    ...
    async def _ticker(self, msg: dict, pair: str, timestamp: float):
        ...
        t = Ticker(self.id, pair, Decimal(m['b'][0]), Decimal(m['a'][0]), None, raw=msg)
    ...
    async def _book(self, msg: dict, pair: str, timestamp: float):
        ...
        if 'as' in msg[0]:
            ...
            if self.max_depth:
                for side in (BID, ASK):
                    n_extras = len(self._l2_book[pair].book[side]) - self.max_depth
                    tail_idx = 0 if side == BID else -1
                    delta[side].extend([
                        (self._l2_book[pair].book[side].popitem(tail_idx)[0], 0)
