...
class Gateio(Feed):
    ...
    async def _ticker(self, msg: dict, timestamp: float):
        ...
        extra_fields = {
            ...
            'volume': Decimal(m.get('quote_volume', 0)),  # this is actually baseVolume
        }
        t = Ticker(
            self.id,
            self.exchange_symbol_to_std_symbol(m['currency_pair']),
            Decimal(m['highest_bid']),
            Decimal(m['lowest_ask']),
            float(msg['time']),
            raw=msg
        )
