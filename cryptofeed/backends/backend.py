...
class BackendUnderlyingIndexCallback:
    async def __call__(self, *, feed: str, symbol: str, underlying_index: Decimal, timestamp: float, receipt_timestamp: float):
        data = {'feed': feed, 'symbol': symbol, 'underlying_index': self.numeric_type(underlying_index), 'receipt_timestamp': receipt_timestamp, 'timestamp': timestamp}
        await self.write(feed, symbol, timestamp, receipt_timestamp, data)
