...
class Callback:
    ...
    async def __call__(self, obj, receipt_timestamp, **kwargs):
        if self.callback is None:
            return
        elif self.is_async:
            await self.callback(obj, receipt_timestamp, **kwargs)
        else:
            loop = asyncio.get_event_loop()
            await loop.run_in_executor(None, self.callback, (obj, receipt_timestamp, kwargs))
